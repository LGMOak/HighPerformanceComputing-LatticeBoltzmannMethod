...
public:
    static void write_results(const Grid& grid, const SimulationParams& params) {
        ...
        std::cout << "  - velocity_profile.csv (profile at x=" << grid.nx()/2 << ")" << std::endl;
...
private:
    static void write_velocity_field(const Grid& grid) {
        ...
        if (!file.is_open()) {
            std::cerr << "Error: Could not open velocity_field.csv for writing" << std::endl;
            return;
        }
    ...
    static void write_velocity_profile(const Grid& grid) {
        ...
        if (!file.is_open()) {
            std::cerr << "Error: Could not open velocity_profile.csv for writing" << std::endl;
            return;
        }
    ...
    static void write_simulation_params(const SimulationParams& params) {
        ...
        if (!file.is_open()) {
            std::cerr << "Error: Could not open simulation_params.csv for writing" << std::endl;
            return;
        }
